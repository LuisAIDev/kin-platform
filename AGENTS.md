...
> **Milestone: KIN 2.0 Alpha 1 — `ARCHITECTURE STABLE`** (tag `v2.0.0-alpha.1`), enmendado por la Fase 5.2.1 (ADR-006…009: runtime consolidado, contexto durable, scoring canonizado), la Fase 5.3 (ADR-010: OpportunityEngine) y la **Fase 5.4 (ADR-011: ReportEngine)**.
...
- **Tests** exist in `kin-backend/src/test/java/`. Run with `cd kin-backend && ./mvnw test`. Currently 274 tests: 5 in `AiEngineServiceTest`, 5 in `ChatOrchestratorServiceImplTest` (SSE via `mockConstruction`), 3 in `KinMethodTest` (full 10-stage pipeline), 5 in `PromptAssemblerTest`, 5 in `ConsultorStageTest`, 4 in `JpaContextRepositoryTest` (JSON round-trip), 7 in `ScoringEngineTest` + `ScoringStageTest`, plus `kin/engine/` (EngineRegistry/EngineExecutor/EngineMetadata/DeterministicId), `pipeline/stage/` (EngineStage, RecommendationStage, RiskStage, OpportunityStage, ReportStage) and `kin/reporting/` (RecommendationEngine/Result/Stage, RiskEngine/Result/Stage, RiskAssembler, RiskAnalyzers, OpportunityEngine/Result/Stage, OpportunityAssembler, OpportunityAnalyzers), `kin/reporting/report/` (ReportEngine, ReportBuilder, ConsultingReport, 10 SectionAssemblers, ReportStage). JaCoCo is configured (`jacoco-maven-plugin`), report at `target/site/jacoco/index.html`. Domain coverage requirement: ≥90% instructions in `kin.reporting` and `kin.engine` (current: 100% in `kin.engine`, 98.6% in `kin.reporting*` aggregate, 100% in `kin.reporting.opportunity`, 99.5% in `kin.reporting.risk`, 98.9% in `kin.scoring`, 99–100% in `kin.reporting.report`). Tests use Mockito + JUnit 5 + reactor-test; the AI engine tests exercise the mock fallback path (no LLM needed). No frontend tests or integration tests yet.
