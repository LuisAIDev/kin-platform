...
> **Milestone: KIN 2.0 Alpha 1 — `ARCHITECTURE STABLE`** (tag `v2.0.0-alpha.1`), enmendado por la Fase 5.2.1 (ADR-006…009: runtime consolidado, contexto durable, scoring canonizado) y la Fase 5.3 (ADR-010: OpportunityEngine).
...
| `pipeline.stage.EngineStage` | Generic engine stage (composition); `ScoringStage`/`RecommendationStage`/`RiskStage`/`OpportunityStage` delegate to it |
