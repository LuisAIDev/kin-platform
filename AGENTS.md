...
> **Milestone: KIN 2.0 Alpha 1 — `ARCHITECTURE STABLE`** (tag `v2.0.0-alpha.1`), enmendado por la Fase 5.2.1 (ADR-006…009: runtime consolidado, contexto durable, scoring canonizado), la Fase 5.3 (ADR-010: OpportunityEngine), la **Fase 5.4 (ADR-011: ReportEngine)** y la **Fase 5.5 (ADR-012: PromptAssembler)**.
...
- **PromptAssembler** is a pure façade (`kin.ai`): `assemble(PromptRequest) → String` delegates to `ConversationPromptBuilder` (CONVERSATION) or `ReportPromptBuilder` (REPORT, via 10 `SectionFormatter` in `kin.ai.prompt`). `PromptRequest.forConversation(...)` / `forReport(...)` enforce the ADR-012 boundary (REPORT only consumes `ConsultingReport`; raw sources are forbidden).
...
- **Pipeline order**: `Analizador → Evaluador → Estratega → Scoring → Recomendaciones → Riesgos → Oportunidades → Reporte → Consultor → Eventos`. `ConsultorStage` runs after `ReportStage` so the LLM receives the `ConsultingReport`; it selects `PromptRequest.forReport(...)` when `decision.shouldGenerateReport()` (throws if the report is missing) and `forConversation(...)` otherwise.
...
- **Tests** exist in `kin-backend/src/test/java/`. Run with `cd kin-backend && ./mvnw test`. Currently 338 tests: 5 in `AiEngineServiceTest`, 5 in `ChatOrchestratorServiceImplTest` (SSE via `mockConstruction`), 3 in `KinMethodTest` (full 10-stage pipeline, incl. REPORT prompt via `ArgumentCaptor`), 8 in `PromptAssemblerTest`, 7 in `ConsultorStageTest` (incl. REPORT mode), 10 in `ReportPromptBuilderTest`, 11 in `ConversationPromptBuilderTest`, 38 across the 10 `SectionFormatter` tests, 4 in `JpaContextRepositoryTest` (JSON round-trip), 7 in `ScoringEngineTest` + `ScoringStageTest`, plus `kin/engine/` (EngineRegistry/EngineExecutor/EngineMetadata/DeterministicId), `pipeline/stage/` (EngineStage, RecommendationStage, RiskStage, OpportunityStage, ReportStage) and `kin/reporting/` (RecommendationEngine/Result/Stage, RiskEngine/Result/Stage, RiskAssembler, RiskAnalyzers, OpportunityEngine/Result/Stage, OpportunityAssembler, OpportunityAnalyzers), `kin/reporting/report/` (ReportEngine, ReportBuilder, ConsultingReport, 10 SectionAssemblers, ReportStage). JaCoCo is configured (`jacoco-maven-plugin`), report at `target/site/jacoco/index.html`. Domain coverage requirement: ≥90% instructions in `kin.reporting`, `kin.engine` and `kin.ai` (current: 100% in `kin.engine`, 100% in `kin.ai`, 98.8% in `kin.ai.prompt`, 99.9% in `kin.ai.prompt.formatter`, 98.6% in `kin.reporting*` aggregate, 100% in `kin.reporting.opportunity`, 99.5% in `kin.reporting.risk`, 98.9% in `kin.scoring`, 99–100% in `kin.reporting.report`). Tests use Mockito + JUnit 5 + reactor-test; the AI engine tests exercise the mock fallback path (no LLM needed). No frontend tests or integration tests yet.
...
| `kin.ai.prompt` | `ConversationPromptBuilder`, `ReportPromptBuilder` (10 `SectionFormatter` en `kin.ai.prompt.formatter`) — frontera ADR-012: REPORT solo consume `ConsultingReport` |
