...
import { enterpriseApi, downloadBlob } from "@/services/enterpriseApi";
...
describe("enterpriseApi.generate", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });

  it("solicita la generación asíncrona con el body correcto", async () => {
    const fetchMock = vi
      .spyOn(globalThis, "fetch")
      .mockResolvedValue(new Response("", { status: 202 }));

    const status = await enterpriseApi.generate("p1", true);

    expect(status).toBe(202);
    const [url, init] = fetchMock.mock.calls[0];
    expect(String(url)).toContain("/enterprise/p1/generate");
    expect((init as RequestInit).method).toBe("POST");
    expect(JSON.parse((init as RequestInit).body as string)).toEqual({ async: true });
  });

  it("trata 409 como una generación ya en curso", async () => {
    vi.spyOn(globalThis, "fetch").mockResolvedValue(
      new Response(JSON.stringify({ message: "en curso" }), { status: 409 }),
    );
  ...
  it("lanza el mensaje del backend cuando la generación falla", async () => {
    vi.spyOn(globalThis, "fetch").mockResolvedValue(
      new Response(JSON.stringify({ message: "Sin contexto de conversación" }), {
        status: 422,
      }),
    );

    await expect(enterpriseApi.generate("p1", true)).rejects.toThrow(
